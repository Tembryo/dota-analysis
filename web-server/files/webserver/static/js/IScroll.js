(function (n, t, i) {
	function u(n, i) {
		...
		this.options.bounceEasing = typeof this.options.bounceEasing == "string" ? r.ease[this.options.bounceEasing] || r.ease.circular : this.options.bounceEasing;
	...
	function f(n, i, r) {
		...
		return r === !0 && (u.style.cssText = "position:absolute;z-index:9999", f.style.cssText = "-webkit-box-sizing:border-box;-moz-box-sizing:border-box;box-sizing:border-box;position:absolute;background:rgba(0,0,0,0.5);border:1px solid rgba(255,255,255,0.9);border-radius:3px"),
		f.className = "iScrollIndicator",
		n == "h" ? (r === !0 && (u.style.cssText += ";height:7px;left:2px;right:2px;bottom:0", f.style.height = "100%"), u.className = "iScrollHorizontalScrollbar") : (r === !0 && (u.style.cssText += ";width:7px;bottom:2px;top:2px;right:1px", f.style.width = "100%"), u.className = "iScrollVerticalScrollbar"),
	...
	function e(i, u) {
		this.wrapper = typeof u.el == "string" ? t.querySelector(u.el) : u.el;
		...
		this.maxPosX = 0;
		this.maxPosY = 0;
		this.options.interactive && (this.options.disableTouch || (r.addEvent(this.indicator, "touchstart", this), r.addEvent(n, "touchend", this)), this.options.disablePointer || (r.addEvent(this.indicator, r.prefixPointerEvent("pointerdown"), this), r.addEvent(n, r.prefixPointerEvent("pointerup"), this)), this.options.disableMouse || (r.addEvent(this.indicator, "mousedown", this), r.addEvent(n, "mouseup", this)));
	...
	r = function () {
		...
		e = function () {
			for (var t = ["t", "webkitT", "MozT", "msT", "OT"], i, n = 0, r = t.length; n < r; n++)
				if (i = t[n] + "ransform", i in f)
		...
		r.prefixPointerEvent = function (t) {
			return n.MSPointerEvent ? "MSPointer" + t.charAt(9).toUpperCase() + t.substr(10) : t
		},
		r.momentum = function (n, t, r, u, f, e) {
			...
			o,
			c;
			return e = e === undefined ? .0006 : e,
			o = n + s * s / (2 * e) * (h < 0 ? -1 : 1),
			c = s / e,
			o < u ? (o = f ? u - f / 2.5 * (s / 8) : u, h = i.abs(o - n), c = h / s) : o > 0 && (o = f ? f / 2.5 * (s / 8) : 0, h = i.abs(n) + o, c = h / s), {
		...
		r.isBadAndroid = /Android /.test(n.navigator.appVersion) && !/Chrome\/\d/.test(n.navigator.appVersion),
		r.extend(r.style = {}, {
			transform : o,
		...
		r.extend(r.ease = {}, {
			...
			elastic : {
				...
				fn : function (n) {
					var t = .22;
					return n === 0 ? 0 : n == 1 ? 1 : .4 * i.pow(2, -10 * n) * i.sin((n - t / 4) * 2 * i.PI / t) + 1
				}
			}
		}),
		r.tap = function (n, i) {
	...
	u.prototype = {
		...
		_start : function (n) {
			if ((r.eventType[n.type] == 1 || n.button === 0) && this.enabled && (!this.initiated || r.eventType[n.type] === this.initiated)) {
		...
		_move : function (n) {
			if (this.enabled && r.eventType[n.type] === this.initiated) {
				...
				s,
				h;
				if (this.pointX = o.pageX, this.pointY = o.pageY, this.distX += t, this.distY += u, s = i.abs(this.distX), h = i.abs(this.distY), !(c - this.endTime > 300) || !(s < 10) || !(h < 10)) {
					if (this.directionLocked || this.options.freeScroll || (this.directionLocked = s > h + this.options.directionLockThreshold ? "h" : h >= s + this.options.directionLockThreshold ? "v" : "n"), this.directionLocked == "h") {
		...
		_end : function (n) {
			...
			if (this.enabled && r.eventType[n.type] === this.initiated) {
				...
				var v = n.changedTouches ? n.changedTouches[0] : n,
				o,
				s,
				...
				if (this.isInTransition = 0, this.initiated = 0, this.endTime = r.getTime(), !this.resetPosition(this.options.bounceTime)) {
					...
					if (this.options.momentum && e < 300 && (o = this.hasHorizontalScroll ? r.momentum(this.x, this.startX, e, this.maxScrollX, this.options.bounce ? this.wrapperWidth : 0, this.options.deceleration) : {
								destination : t,
								duration : 0
		...
		scrollToElement : function (n, t, u, f, e) {
			if (n = n.nodeType ? n : this.scroller.querySelector(n), n) {
				var o = r.offset(n);
				...
				t = t === undefined || t === null || t === "auto" ? i.max(i.abs(this.x - o.left), i.abs(this.y - o.top)) : t;
		...
		_transitionTime : function (n) {
			if (n = n || 0, this.scrollerStyle[r.style.transitionDuration] = n + "ms", !n && r.isBadAndroid && (this.scrollerStyle[r.style.transitionDuration] = "0.001s"), this.indicators)
				for (var t = this.indicators.length; t--; )
					this.indicators[t].transitionTime(n)
		},
		...
		getComputedPosition : function () {
			var t = n.getComputedStyle(this.scroller, null),
			i,
			u;
		...
		_initIndicators : function () {
			...
			for (this.indicators = [], this.options.scrollbars && (this.options.scrollY && (t = {
							...
							fade : this.options.fadeScrollbars,
							listenY : !1
						}, this.wrapper.appendChild(t.el), n.push(t))), this.options.indicators && (n = n.concat(this.options.indicators)), u = n.length; u--; )
				this.indicators.push(new e(this, n[u]));
		...
		_wheel : function (n) {
			if (this.enabled) {
				...
				f,
				e = this;
				if (this.wheelTimeout === undefined && e._execEvent("scrollStart"), clearTimeout(this.wheelTimeout), this.wheelTimeout = setTimeout(function () {
							e._execEvent("scrollEnd");
							e.wheelTimeout = undefined
						}, 400), "deltaX" in n)
					n.deltaMode === 1 ? (r = -n.deltaX * this.options.mouseWheelSpeed, t = -n.deltaY * this.options.mouseWheelSpeed) : (r = -n.deltaX, t = -n.deltaY);
				else if ("wheelDeltaX" in n)
					r = n.wheelDeltaX / 120 * this.options.mouseWheelSpeed, t = n.wheelDeltaY / 120 * this.options.mouseWheelSpeed;
				else if ("wheelDelta" in n)
					r = t = n.wheelDelta / 120 * this.options.mouseWheelSpeed;
				else if ("detail" in n)
					r = t = -n.detail / 3 * this.options.mouseWheelSpeed;
				else
					return;
				if (r *= this.options.invertWheelDirection, t *= this.options.invertWheelDirection, this.hasVerticalScroll || (r = t, t = 0), this.options.snap) {
			...
			this.on("refresh", function () {
				...
				if (this.pages = [], this.wrapperWidth && this.wrapperHeight && this.scrollerWidth && this.scrollerHeight) {
					if (this.options.snap === !0)
						for (o = i.round(c / 2), s = i.round(l / 2); r > -this.scrollerWidth; ) {
							for (this.pages[n] = [], f = 0, u = 0; u > -this.scrollerHeight; )
								this.pages[n][f] = {
									x : i.max(r, this.maxScrollX),
									y : i.max(u, this.maxScrollY),
									width : c,
									height : l,
									cx : r - o,
									cy : u - s
						...
						for (t = this.options.snap, f = t.length, h = -1; n < f; n++)
							(n === 0 || t[n].offsetLeft <= t[n - 1].offsetLeft) && (e = 0, h++), this.pages[e] || (this.pages[e] = []), r = i.max(-t[n].offsetLeft, this.maxScrollX), u = i.max(-t[n].offsetTop, this.maxScrollY), o = r - i.round(t[n].offsetWidth / 2), s = u - i.round(t[n].offsetHeight / 2), this.pages[e][h] = {
								x : r,
								y : u,
		...
		_nearestSnap : function (n, t) {
			...
			f = this.pages.length,
			u = 0;
			if (i.abs(n - this.absStartX) < this.snapThresholdX && i.abs(t - this.absStartY) < this.snapThresholdY)
				return this.currentPage;
			for (n > 0 ? n = 0 : n < this.maxScrollX && (n = this.maxScrollX), t > 0 ? t = 0 : t < this.maxScrollY && (t = this.maxScrollY); r < f; r++)
			...
			return r == this.currentPage.pageX && (r += this.directionX, r < 0 ? r = 0 : r >= this.pages.length && (r = this.pages.length - 1), n = this.pages[r][0].x),
			u == this.currentPage.pageY && (u += this.directionY, u < 0 ? u = 0 : u >= this.pages[0].length && (u = this.pages[0].length - 1), t = this.pages[0][u].y), {
				x : n,
				y : t,
				pageX : r,
				pageY : u
		...
		goToPage : function (n, t, r, u) {
			...
			e = this.pages[n][t].y;
			r = r === undefined ? this.options.snapSpeed || i.max(i.max(i.min(i.abs(f - this.x), 1e3), i.min(i.abs(e - this.y), 1e3)), 300) : r;
			this.currentPage = {
				x : f,
				y : e,
				pageX : n,
		...
		_initKeys : function () {
			...
			if (typeof this.options.keyBindings == "object")
				for (t in this.options.keyBindings)
					typeof this.options.keyBindings[t] == "string" && (this.options.keyBindings[t] = this.options.keyBindings[t].toUpperCase().charCodeAt(0));
			else
		...
		_key : function (n) {
			if (this.enabled) {
				...
				s = this.keyTime || 0,
				e;
				this.options.useTransition && this.isInTransition && (e = this.getComputedPosition(), this._translate(i.round(e.x), i.round(e.y)), this.isInTransition = !1);
				this.keyAcceleration = o - s < 200 ? i.min(this.keyAcceleration + .25, 50) : 0;
				switch (n.keyCode) {
		...
		_animate : function (n, t, i, u) {
			function c() {
				...
				if (a >= l) {
					f.isAnimating = !1;
					f._translate(n, t);
					f.resetPosition(f.options.bounceTime) || f._execEvent("scrollEnd");
					return
				}
				a = (a - h) / i;
				v = u(a);
	...
	e.prototype = {
		...
		refresh : function () {
			...
			var n = this.wrapper.offsetHeight;
			this.options.listenX && (this.wrapperWidth = this.wrapper.clientWidth, this.options.resize ? (this.indicatorWidth = i.max(i.round(this.wrapperWidth * this.wrapperWidth / (this.scroller.scrollerWidth || this.wrapperWidth || 1)), 8), this.indicatorStyle.width = this.indicatorWidth + "px") : this.indicatorWidth = this.indicator.clientWidth, this.maxPosX = this.wrapperWidth - this.indicatorWidth, this.options.shrink == "clip" ? (this.minBoundaryX = -this.indicatorWidth + 8, this.maxBoundaryX = this.wrapperWidth - 8) : (this.minBoundaryX = 0, this.maxBoundaryX = this.maxPosX), this.sizeRatioX = this.options.speedRatioX || this.scroller.maxScrollX && this.maxPosX / this.scroller.maxScrollX);
			this.options.listenY && (this.wrapperHeight = this.wrapper.clientHeight, this.options.resize ? (this.indicatorHeight = i.max(i.round(this.wrapperHeight * this.wrapperHeight / (this.scroller.scrollerHeight || this.wrapperHeight || 1)), 8), this.indicatorStyle.height = this.indicatorHeight + "px") : this.indicatorHeight = this.indicator.clientHeight, this.maxPosY = this.wrapperHeight - this.indicatorHeight, this.options.shrink == "clip" ? (this.minBoundaryY = -this.indicatorHeight + 8, this.maxBoundaryY = this.wrapperHeight - 8) : (this.minBoundaryY = 0, this.maxBoundaryY = this.maxPosY), this.maxPosY = this.wrapperHeight - this.indicatorHeight, this.sizeRatioY = this.options.speedRatioY || this.scroller.maxScrollY && this.maxPosY / this.scroller.maxScrollY);
			this.updatePosition()
		},
	...
	u.utils = r;
	typeof module != "undefined" && module.exports ? module.exports = u : n.IScroll = u
})(window, document, Math), function (n) {
	"use strict";
	var t = function (n, i, r) {
		...
		if (u.onerror = i, u.onload = function () {
			...
			f = n, r && r.crossOrigin && (u.crossOrigin = r.crossOrigin);
		else
			return !1;
	...
	t.getTransformedOptions = function (n, t) {
		...
		return i.crop = !0,
		f = n.naturalWidth || n.width,
		e = n.naturalHeight || n.height,
		f / e > r ? (i.maxWidth = e * r, i.maxHeight = e) : (i.maxWidth = f, i.maxHeight = f / r),
		i
	};
	t.renderImageToCanvas = function (n, t, i, r, u, f, e, o, s, h) {
		return n.getContext("2d").drawImage(t, i, r, u, f, e, o, s, h),
		n
	};
...
(function (n) {
	...
	if (window.navigator && window.navigator.platform && /iP(hone|od|ad)/.test(window.navigator.platform)) {
		...
		n.detectVerticalSquash = function (n, t) {
			...
			i,
			h;
			for (t && (r /= 2), f.width = 1, f.height = r, o.drawImage(n, 0, 0), s = o.getImageData(0, 0, 1, r).data, u = 0, e = r, i = r; i > u; )
				h = s[(i - 1) * 4 + 3], h === 0 ? e = i : u = i, i = e + u >> 1;
			return i / r || 1
		};
			...
			if (r._type === "image/jpeg") {
				...
				if (v.width = a, v.height = a, k.save(), y = n.detectSubsampling(r), y && (u /= 2, f /= 2, e /= 2, o /= 2), p = n.detectVerticalSquash(r, y), y || p !== 1) {
					for (f *= p, c = Math.ceil(a * c / e), l = Math.ceil(a * l / o / p), h = 0, b = 0; b < o; ) {
						for (s = 0, w = 0; w < e; )
							d.clearRect(0, 0, a, a), d.drawImage(r, u, f, e, o, -w, -b, e, o), k.drawImage(v, 0, 0, a, a, s, h, c, l), w += a, s += c;
						b += a;
						h += l
					}
					return k.restore(),
...
(function (n) {
	...
	n.getTransformedOptions = function (t, i) {
		...
		return u.orientation > 4 && (f.maxWidth = u.maxHeight, f.maxHeight = u.maxWidth, f.minWidth = u.minHeight, f.minHeight = u.minWidth, f.sourceWidth = u.sourceHeight, f.sourceHeight = u.sourceWidth),
		f
	}
}), function (n) {
	"use strict";
...
(function (n) {
	...
	n.getExifThumbnail = function (n, t, i) {
		...
		for (u = [], r = 0; r < i; r += 1)
			f = n.getUint8(t + r), u.push((f < 16 ? "0" : "") + f.toString(16));
	...
	n.parseExifTags = function (n, t, i, r, u) {
		...
		for (f = 0; f < e; f += 1)
			this.parseExifTag(n, t, i + 2 + 12 * f, r, u);
